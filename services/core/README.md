...
Local HTTP API (`FastAPI` + `uvicorn`). See repository root `README.md` and `docs/spec/http-api.md`. **MP1:** `edge_base_url`, `deployment_mode`, optional edge TLS + `edge_ingest_path_*` in config; admin §3.11; **`host_edge` / `hybrid`:** `POST /retrieve` ([`remote_retrieval.py`](memoryagent/remote_retrieval.py)) + `POST /ingest` ([`remote_ingest.py`](memoryagent/remote_ingest.py)) after local writes — memory always (host_edge awaits edge; hybrid fires memory to edge in a background task); **file/mirror** to edge only when host paths map to Node paths. Shared httpx `verify` via [`edge_http.py`](memoryagent/edge_http.py). Chat and watcher use the same `IngestBackend` as HTTP (`bind_ingest_for_routing` on [`rag_service.py`](memoryagent/rag_service.py)). Rebind on `PATCH /config` when any edge-related field changes.
