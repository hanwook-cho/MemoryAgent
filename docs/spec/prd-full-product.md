...
| **Deliverables** | **Done:** PR-1 adapters, PR-2 health `deployment` block; `edge_base_url` + `PATCH /config`; edge `GET /health` stub ([`node_client.py`](../../services/core/memoryagent/node_client.py)); health + chat `meta`; admin §3.11; **remote retrieval** ([`remote_retrieval.py`](../../services/core/memoryagent/remote_retrieval.py) `POST /retrieve`, `HostEdge`/`Hybrid` retrieval backends); **memory ingest fan-out** ([`remote_ingest.py`](../../services/core/memoryagent/remote_ingest.py) `POST /ingest` after local success, `HostEdgeIngestBackend` / `HybridIngestBackend`); `PATCH /config` rebinds retrieval + ingest. **Note:** chat-internal `ingest_memory` (save prefix) still uses host `RagService` only until wired to `IngestBackend`. **Next:** remote file/mirror ingest, hybrid ingest policy, TLS pinning. |
