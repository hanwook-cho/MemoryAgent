...
| **Deliverables** | **Done:** PR-1 adapters, PR-2 health `deployment` block; `edge_base_url` + `PATCH /config` (+ edge TLS + path-map fields); edge `GET /health` ([`node_client.py`](../../services/core/memoryagent/node_client.py) with shared [`edge_http.py`](../../services/core/memoryagent/edge_http.py) verify); health + chat `meta`; admin §3.11; **remote retrieval** (`POST /retrieve`, `HostEdge`/`Hybrid`); **ingest** — chat save prefix + tools + watcher route through `IngestBackend`; memory `POST /ingest` after local write; **file/mirror** `POST /ingest` `kind=file` when `edge_ingest_path_*` maps host path → Node path; **hybrid** memory edge push is non-blocking (background task). **Next:** SPKI / cert pinning beyond CA bundle + `verify=False` lab flag; richer hybrid ingest than memory-async-only. |
