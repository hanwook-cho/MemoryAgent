...
| **Deliverables** | **Done:** PR-1 adapters, PR-2 health `deployment` block; `edge_base_url` + `PATCH /config`; edge `GET /health` stub ([`node_client.py`](../../services/core/memoryagent/node_client.py)); health + chat `meta` ([`deployment_runtime.py`](../../services/core/memoryagent/deployment_runtime.py)); admin §3.11; **remote retrieval:** [`remote_retrieval.py`](../../services/core/memoryagent/remote_retrieval.py) `POST /retrieve` + [`backends.py`](../../services/core/memoryagent/backends.py) `HostEdgeRetrievalBackend` / `HybridRetrievalBackend` + `build_runtime_backends`; chat + `/memory/search` use same backend; `PATCH /config` rebinds retrieval when mode or edge URL changes. **Next:** remote `IngestBackend`, hybrid policy tuning, TLS beyond ping + cert pinning. |
